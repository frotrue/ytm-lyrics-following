...
- 제목과 가수가 정확히 일치하는 기록을 우선 사용합니다. `Oort Cloud (오르트구름)`처럼 서로 다른 문자 체계가 괄호로 병기된 이름은 명시적 별칭으로만 인정하고, 양쪽 길이가 2초 이내로 확인될 때만 사용합니다. Live·Remix·Acoustic·라이브·리믹스·버전 같은 버전 표기와 feat./&/with 협업 표기는 별칭으로 취급하지 않으므로 다른 버전이나 다른 가수를 대신 선택하지 않습니다.
- 정확 조회가 실패하면 앨범 조건을 뺀 검색과 제목만 검색까지 최대 5회로 단계적으로 넓히고, 모든 후보를 원래 제목·가수·버전·길이로 다시 검증합니다. 점수가 같은 후보는 순서로 임의 선택하지 않고 결과 없음으로 처리합니다.
- 일반 가사를 찾은 뒤에도 같은 범위 안에서 동기화 가사를 한 번 더 찾고, 추가 검색이 비어 있거나 모호하거나 실패하면 확보한 일반 가사를 그대로 사용합니다.
...
가사 조회 시 현재 곡의 제목, 아티스트, 가능한 경우 앨범과 곡 길이를 **https://lrclib.net**으로 전송합니다. 재생 위치, 계정 정보, 쿠키, 페이지 URL은 조회 데이터로 전송하지 않습니다. 네트워크 통신 자체에 수반되는 IP 주소는 LRCLIB에서 볼 수 있습니다. 결과는 확장 프로그램의 로컬 저장소에 캐시하며 별도 분석/추적 서비스는 없습니다. 캐시 항목에는 매칭 정책 버전이 함께 기록되고, 이전 정책으로 저장된 일반/결과 없음 항목은 로드할 때 무시합니다. 동기화·연주곡 항목은 그대로 유지하므로 저장소를 직접 지울 필요가 없습니다.
...
동일 제목의 라이브·리믹스·뮤직비디오 등은 길이가 달라 조회가 거절되거나 가사가 없을 수 있습니다. 잘못된 버전의 가사를 임의로 선택하지 않는 쪽을 우선합니다. 별칭 인정 범위는 한글·라틴 문자가 한 번만 병기된 형태로 제한하며, 추측 음역·번역이나 곡별 예외 목록은 사용하지 않습니다. 병기 안쪽에 with/feat 같은 단어가 들어간 이름은 별칭으로 보지 않아 정확 일치만 허용합니다.
