...
- 가사 서버가 일시적인 HTTP 429/502/503/504 오류를 보내면 요청을 최대 두 번 더 시도합니다. 실패가 지속되면 오류 안내의 **Retry** 버튼으로 다시 조회할 수 있습니다. 일시적인 서버 오류는 가사 없음으로 캐시하지 않습니다.
...
현재 검증 결과: 자동 테스트 28개 통과, Firefox 확장 검사 오류·경고 0개. 실제 페이지에서 Stay This Way(73줄) ↔ Love Me Back(53줄) 전환, 앞뒤 탐색, 실제 재생, 가사 탭 재진입, 현재 줄 중앙 정렬을 확인했습니다. v0.1.1에서는 마우스·Enter·Space 가사 이동, 재생/정지 상태 유지, 2560px·1280px 화면의 커버 정렬과 800px 화면의 세로 배치도 확인했습니다. v0.1.2에서는 일시적인 503 응답의 재시도를 자동 테스트로 검증했고, 실제 페이지에서 연속 503을 주입한 뒤 Retry로 같은 곡의 가사가 복구되는 것을 확인했습니다. 다음 곡 조회 중 실제 503이 발생한 경우에도 Retry 후 정상 응답으로 복구됐습니다. 테스트 브라우저에 표시된 패널은 임시로 삽입한 것이므로 새로고침 후에도 쓰려면 위 설치 절차를 따라 확장을 로드해야 합니다.
